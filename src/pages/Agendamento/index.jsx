  ...
  useEffect(() => {
    const fetchHorariosDisponiveis = async () => {
      ...
      try {
        ...
        const querySnapshot = await getDocs(q);
        const agendamentosExistentes = querySnapshot.docs.map(
          (doc) => doc.data().hora
        );
        ...
        const horariosFiltrados =
          horariosDisponiveisPorQuadra[selectedQuadra]?.filter(
            (horario) =>
              ...
              new Date(
                `${selectedDate.toISOString().split("T")[0]}T${horario}`
              ) > today
          ) || [];
          ...
          agendamentos.map((agendamento) => (
            <div key={agendamento.id} className={styles.agendamentoItem}>
              <div className={styles.agendamentoInfo}>
                <img
                  src={
                    quadras.find((q) => q.name === agendamento.quadra)?.image
                  }
                  alt={agendamento.quadra}
                  className={styles.agendamentoImage}
                />
